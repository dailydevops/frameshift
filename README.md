...
- **`Mutations`** and **`Mutations/Operators`** — the 25 mutation operators, the registry that indexes them by the syntax kinds they support, the generator that applies them, and the compiler that verifies each mutant.
- **`Mutations/RegularExpressions`** — the building blocks for reasoning about regular expression patterns: an options-aware pattern tokenizer, the locator that recognises a pattern in source, and a validity check that answers whether a rewritten pattern is still a legal pattern. The four operators of the regular-expression pattern family are built on them.
...
- 25 mutation operators covering arithmetic, relational, equality, logical, bitwise, unary, increment/decrement, conditional, null-coalescing, boolean, nullable boolean, numeric and string literal mutations, plus a culture-sensitivity family: `StringComparison`, `StringComparer`, `CultureInfo`, `IFormatProvider` argument removal, case conversion and `RegexOptions` flags, plus a regular-expression pattern family that rewrites the pattern text itself: anchors, quantifiers, groups and alternation.
