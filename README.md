...
- **`Mutations`** and **`Mutations/Operators`** — the 21 mutation operators, the registry that indexes them by the syntax kinds they support, the generator that applies them, and the compiler that verifies each mutant.
- **`Mutations/RegularExpressions`** — internal building blocks for reasoning about regular expression patterns: an options-aware pattern tokenizer, the locator that recognises a pattern in source, and a validity check. No mutation operator is built on them yet, so nothing here is user-visible.
...
- 21 mutation operators covering arithmetic, relational, equality, logical, bitwise, unary, increment/decrement, conditional, null-coalescing, boolean, nullable boolean, numeric and string literal mutations, plus a culture-sensitivity family: `StringComparison`, `StringComparer`, `CultureInfo`, `IFormatProvider` argument removal, case conversion and `RegexOptions` flags.
