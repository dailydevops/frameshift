...
/// <summary>
/// The test-side analyzer of Frameshift for MSTest. It runs on an MSTest test project, discovers the
/// test methods of the compilation, walks the code reachable from them and determines which production
...
/// </para>
/// <para>
/// The analyzer shuts down entirely — reporting nothing whatsoever — unless MSTest is detected in the
/// compilation and at least one MSTest test method is actually discovered. Every framework gets its own
/// analyzer built on the same shared analysis, and each one stays silent on compilations that are not
/// its own.
/// </para>
/// </remarks>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class MSTestTestSurfaceAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void OnCompilation(CompilationAnalysisContext context) =>
        TestSurfaceAnalysis.Execute(context, MSTestTestFrameworkProbe.Instance);
}
