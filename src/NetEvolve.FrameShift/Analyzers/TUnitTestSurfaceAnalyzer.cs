...
/// <summary>
/// The test-side analyzer of FrameShift for TUnit. It runs on a TUnit test project, discovers the test
...
/// </para>
/// <para>
/// The analyzer shuts down entirely — reporting nothing whatsoever — unless TUnit is detected in the
/// compilation and at least one TUnit test method is actually discovered. Every framework gets its own
/// analyzer built on the same shared analysis, and each one stays silent on compilations that are not
/// its own.
/// </para>
/// </remarks>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class TUnitTestSurfaceAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void OnCompilation(CompilationAnalysisContext context) =>
        TestSurfaceAnalysis.Execute(context, TUnitTestFrameworkProbe.Instance);
}
